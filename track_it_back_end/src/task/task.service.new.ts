...
export class TaskService {
	...
	async create(dto: TaskDto, createdBy: string, columnId: string) {
		...
		// Отправляем уведомления о создании задачи
		if (createdTask) {
			await this.notificationIntegrationService.notifyTaskCreated(
				createdTask.column.board.projectId,
				createdTask.id,
				createdTask.title,
				createdBy
			);
	...
	async update(dto: Partial<TaskDto>, taskId: string, createdBy: string) {
		...
		// Отправляем уведомления о обновлении задачи
		if (finalTask) {
			await this.notificationIntegrationService.notifyTaskUpdated(
				finalTask.column.board.projectId,
				finalTask.id,
				finalTask.title,
				createdBy
			);
	...
	async delete(taskId: string, userId: string) {
		...
		// Сохраняем информацию о задаче для уведомлений
		const taskInfo = {
			id: existingTask.id,
		...
		await this.notificationIntegrationService.notifyTaskDeleted(
			taskInfo.projectId,
			taskInfo.title,
			userId
		);
